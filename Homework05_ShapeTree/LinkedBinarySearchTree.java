...
public class LinkedBinarySearchTree <T extends Comparable<T>> {
	...
	// finds the minimum value in a tree
	public Node findMaxInTree(Node aNode) {
		if (aNode == null)
			return null;
		
		// if no right child, node is largest in tree
		if (aNode.rightChild == null)
			return aNode;
		else
			// search for largest value in right child
			return findMaxInTree(aNode.rightChild);
	}

	public T findMaxAreaInTree() {
		return findMaxInTree(root).data;
	}

	public void deleteValuesGreaterThan(T aData) {
		if (root == null)
			return;

		// this node's value was less than target value, no nodes in tree are larger than parameter
		if (findMaxInTree(root).data.compareTo(aData) < 0) {
			System.out.println("All values less than parameter");
			return;
		}
		// this node's value was greater than target value, remove it and call again
		else if (findMaxInTree(root).data.compareTo(aData) > 0) {
			remove(findMaxInTree(root).data);
			deleteValuesGreaterThan(aData);
		}
	}
