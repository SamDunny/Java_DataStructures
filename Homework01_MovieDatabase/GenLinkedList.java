...
public class GenLinkedList <T> {
    ...
    // removes data from parameter, WARNING resets current
    public void remove(T aData) {
        // set current to sought item, remove it
        reset();
        while(!current.data.equals(aData) && current.link != null) {
            gotoNext();
        }
        // System.out.println("Removing: " + current.data);
        removeCurrent();

        // sets current back to beginning
        reset();       
    }
    ...
    public boolean isInList(T aData) {
        // start an iterator at the head
        ListNode temp = head;

        // as long as the node isn't null, compare and move to the next element
        while (temp != null) {
